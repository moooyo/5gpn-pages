...
/**
 * Validate a relay endpoint as a bare, fully qualified ASCII domain name.
 * The returned reason is stable and intended for mapping to UI copy.
 */
export function validateRelayHost(input) {
  const raw = String(input || '').trim();
  if (!raw) return { valid: false, value: '', reason: 'required' };
  if (raw.includes('://')) return { valid: false, value: raw, reason: 'scheme' };
  if (raw.includes(':')) return { valid: false, value: raw, reason: 'port' };
  if (/[/?#\\]/.test(raw)) return { valid: false, value: raw, reason: 'suffix' };
  ...
  const validTopLevel = /^(?:[a-z]{2,63}|xn--[a-z0-9-]{2,59})$/.test(topLevel);

  if (value.length > 253 || labels.length < 2 || !labels.every(validLabel) || !validTopLevel) {
